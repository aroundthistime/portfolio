...
하지만 여전히 각 타이머 UI에서 사용하는 interval이 분산되어 동기성이 느껴지는 문제가 있었고, 이는 프로덕트 완성도를 저해한다고 판단했습니다. 이에 따라 동일한 주기를 갖는 반복 작업들이 하나의 interval 객체를 공유할 수 있도록 구조를 개선했습니다. 전역 단위의 동기화 interval 로직을 활용해 모든 타이머 UI들이 같은 tick 이벤트를 기준으로 상태를 업데이트하도록 했고, 그 외에도 코드에서 사용하는 인터벌 로직에 이를 적용해 interval 객체 수 자체를 줄임으로써 성능 상의 이점 역시 함께 달성할 수 있었습니다.`
      }
    ],
    period: {
      startDate: '2024-03',
